...
type AWSAdapterConfigReconciler struct {
	client.Client
	PolicyReportRESTClient rest.Interface
...
func (r *AWSAdapterConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if !cmp.Equal(objOld.Status, securityv1alpha1.AWSAdapterConfigStatus{}) {
	...
	if x, err := svc.ListNodegroups(context.TODO(), &eks.ListNodegroupsInput{ClusterName: objOld.Spec.Name}); err == nil {
		for _, y := range x.Nodegroups {
			if z, err := svc.DescribeNodegroup(context.TODO(), &eks.DescribeNodegroupInput{ClusterName: objOld.Spec.Name, NodegroupName: &y}); err == nil {
				...
				if z.Nodegroup.LaunchTemplate != nil {
					launchTemplate = &securityv1alpha1.EC2LaunchTemplate{
						ID:      z.Nodegroup.LaunchTemplate.Id,
						Name:    z.Nodegroup.LaunchTemplate.Name,
						Version: z.Nodegroup.LaunchTemplate.Version,
					}
				}

				healthIssues := []*securityv1alpha1.EKSNodeGroupHealthIssue{}
				for _, issue := range z.Nodegroup.Health.Issues {
				...
				if z.Nodegroup.RemoteAccess != nil {
					remoteAccessConfig = new(securityv1alpha1.EKSNodeGroupRemoteAccessConfig)
					remoteAccessConfig.Ec2SshKey = z.Nodegroup.RemoteAccess.Ec2SshKey
					remoteAccessConfig.SourceSecurityGroups = z.Nodegroup.RemoteAccess.SourceSecurityGroups
				}
				objNew.Status.EKSCluster.Compute.NodeGroups = append(objNew.Status.EKSCluster.Compute.NodeGroups, &securityv1alpha1.EKSNodeGroup{
					...
					ScalingConfig: &securityv1alpha1.EKSNodeGroupScalingConfig{
						DesiredSize: z.Nodegroup.ScalingConfig.DesiredSize,
						MinSize:     z.Nodegroup.ScalingConfig.MinSize,
						MaxSize:     z.Nodegroup.ScalingConfig.MaxSize,
					},
					LaunchTemplate:     launchTemplate,
					Status:             string(z.Nodegroup.Status),
					AMIReleaseVersion:  z.Nodegroup.ReleaseVersion,
					HealthIssues:       healthIssues,
					AMIType:            string(z.Nodegroup.AmiType),
					CapacityType:       string(z.Nodegroup.CapacityType),
					CreatedAt:          z.Nodegroup.CreatedAt.String(),
					DiskSize:           z.Nodegroup.DiskSize,
					InstanceTypes:      z.Nodegroup.InstanceTypes,
					NodegroupArn:       z.Nodegroup.NodegroupArn,
					NodeRole:           z.Nodegroup.NodeRole,
					RemoteAccessConfig: remoteAccessConfig,
					Resources: &securityv1alpha1.EKSNodeGroupResources{
						RemoteAccessSecurityGroup: z.Nodegroup.Resources.RemoteAccessSecurityGroup,
						AutoScalingGroups:         autoScalingGroups,
					},
					Subnets: z.Nodegroup.Subnets,
					Tags:    z.Nodegroup.Tags,
					Taints:  taints,
					Labels:  z.Nodegroup.Labels,
					UpdateConfig: &securityv1alpha1.EKSNodeGroupUpdateConfig{
						MaxUnavailable:           z.Nodegroup.UpdateConfig.MaxUnavailable,
						MaxUnavailablePercentage: z.Nodegroup.UpdateConfig.MaxUnavailablePercentage,
					},
				})
			} else {
				l.Error(err, fmt.Sprintf("error describing nodegroup '%s'", y))
				return r.updateLastPollStatusFailure(ctx, objOld, fmt.Sprintf("error describing nodegroup '%s'", y), err, &l, time.Now())
			}
		}
	} else {
	...
	resp, err := r.PolicyReportRESTClient.Get().AbsPath("/apis/wgpolicyk8s.io/v1alpha2/policyreports/").DoRaw(context.TODO())
	if err != nil {
		l.Error(err, "error encountered")
	}

	var msi map[string]interface{}
	err = json.Unmarshal(resp, &msi)
	if err != nil {
		l.Error(err, "error encountered")
	}

	if msi["items"] != nil {
		kpvs := new([]securityv1alpha1.KyvernoPolicyViolation)
		for _, u := range msi["items"].([]interface{}) {
			var pr v1alpha2.PolicyReport
			prbytes, err := json.Marshal(u)
			if err != nil {
				l.Error(err, "error encountered")
			}
			...
			for _, v := range pr.Results {
				for _, w := range v.Resources {
					if v.Result == "fail" {
						// for _, x := range objNew.Status.KyvernoPolicyViolations {
						kpv := securityv1alpha1.KyvernoPolicyViolation{
						...
						l.Info(fmt.Sprintf("DEBUGGING kpv: %+v", kpv))
						*kpvs = append(*kpvs, kpv)
						// }
					}
				}
			}
		...
		resolveKPV := new([]securityv1alpha1.KyvernoPolicyViolation)
		for _, u := range objNew.Status.KyvernoPolicyViolations {
			foundKPV := new(bool)
			for _, v := range *kpvs {
				if u == v {
					*foundKPV = true
			...
			if !*foundKPV {
				*resolveKPV = append(*resolveKPV, u)
			}
		}
		...
		for _, u := range *kpvs {
			noteText := fmt.Sprintf("APIVersion: %s\nKind: %s\nNamespace: %s\nName: %s\nUID: %s\nPolicy: %s\nRule: %s\nMessage: %s",
				...
				u.Rule,
				u.Message)
			generatorId := r.AwsAccountId + "/default"
			productArn := "arn:aws:securityhub:" + *objNew.Spec.Region + ":" + r.AwsAccountId + ":product/" + generatorId
			findingId := productArn + "/finding/" + fmt.Sprintf("%x", sha1.Sum([]byte(noteText)))

			if !FindingExists(ctx, cfg, findingId) {
				l.Info("+=+=+ FINDING DOES NOT EXIST +=+=+")
				CreateSecurityHubFinding(ctx, objNew.Namespace, *objNew.Spec.Name, *objNew.Spec.Region, r.CompanyName, r.AwsAccountId, r.SecurityHubFindingType, cfg, u)
			}
		}
		objNew.Status.KyvernoPolicyViolations = *kpvs
	}
	...
	if !cmp.Equal(objNew.Status.EKSCluster, objOld.Status.EKSCluster) || !cmp.Equal(objNew.Status.KyvernoPolicyViolations, objOld.Status.KyvernoPolicyViolations) {
