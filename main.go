...
func main() {
	...
	flag.StringVar(&awsAccountId, "aws-account-id", "", "The AWS account's 12-digit ID.")
	opts := zap.Options{
		Development: true,
	}
	...
	polrGvk := schema.GroupVersionKind{
		Group:   "",
		Version: "v1alpha2",
		Kind:    "PolicyReport",
	}

	restClient, err := apiutil.RESTClientForGVK(polrGvk, false, mgr.GetConfig(), serializer.NewCodecFactory(mgr.GetScheme()))
	if err != nil {
		setupLog.Error(err, "unable to create REST client")
	}

	restClient.Get().AbsPath()

	if err = (&controllers.AWSAdapterConfigReconciler{
		Client:                 mgr.GetClient(),
		PolicyReportRESTClient: restClient,
		Scheme:                 mgr.GetScheme(),
		RequeueInterval:        time.Duration(syncPeriod) * time.Minute,
		AwsAccountId:           awsAccountId,
		CompanyName:            COMPANY_NAME,
		SecurityHubFindingType: "Software and Configuration Checks",
